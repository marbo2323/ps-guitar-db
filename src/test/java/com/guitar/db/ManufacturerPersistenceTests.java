...
public class ManufacturerPersistenceTests {
	...
	public void testFindActiveTrue() {
		List<Manufacturer> manufs = manufacturerJpaRepository.findByActiveTrue();
		assertEquals(1, manufs.size());
	}

	@Test
	public void testFindActiveFalse() {
		List<Manufacturer> manufs = manufacturerJpaRepository.findByActiveFalse();
		assertEquals(1, manufs.size());
	}
